...
python legacy_model_merger.py merge \
    --backend fsdp \
    --local_dir checkpoints/DoGe/doge_stage1/global_step_150/actor \
    --target_dir checkpoints/model_example
```
...
class BaseModelMerger(ABC):
    def __init__(self, config: ModelMergerConfig):
        self.config = config
        self.hf_model_config_path = os.path.join(config.hf_model_config_path, "huggingface")
        ...
        if config.hf_model_path:
            ...
            self.hf_model_config_path = os.path.join(config.hf_model_path, "huggingface")

        # print('pppppppppppppppppppppppppppppppppppp')
        # 直接指定 config.json 文件的路径
        # self.hf_model_config_path = os.path.join(self.hf_model_config_path, "huggingface")
        # print(f"Config path: {hf_model_config_path}")
